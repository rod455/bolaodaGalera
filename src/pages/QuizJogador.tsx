...
const QuizJogador = () => {
  ...
  useEffect(() => {
    window.scrollTo(0, 0);
    if (startDirect) {
      trackEvent("quiz_jogador_start", { source: "lp" });
    }
  }, [startDirect]);

  const handleStart = () => {
    trackEvent("quiz_jogador_start", { source: "app" });
    setStep("pergunta");
    setPerguntaIdx(0);
    setRespostas([]);
  ...
  const handleResposta = (opcaoIdx: number) => {
    ...
    if (novasRespostas.length >= PERGUNTAS_JOGADOR.length) {
      ...
      trackEvent("quiz_jogador_complete", { resultado: resultado.id });
  ...
  const handleAdGate = async () => {
    if (needsAd) {
      trackEvent("quiz_jogador_ad_shown", {});

...
import { JOGADORES, PERGUNTAS_JOGADOR, calcularJogador, type Jogador } from "@/lib/quiz-jogador-data";
import { shareViaWhatsApp } from "@/lib/utils";

type QuizStep = "intro" | "pergunta" | "calculando" | "ad-gate" | "resultado";

const PREVIEW_POSICOES = [
  { emoji: "🎯", nome: "Centroavante" }, { emoji: "🎩", nome: "Meia Armador" },
  { emoji: "🧊", nome: "Zagueiro Elegante" }, { emoji: "🌀", nome: "Falso 9", blur: true },
  { emoji: "🐴", nome: "Box-to-Box", blur: true }, { emoji: "😴", nome: "Goleiro Tranquilo", blur: true },
  { emoji: "🚀", nome: "Ponta Direita", blur: true }, { emoji: "🤷", nome: "Coringa", blur: true },
  { emoji: "⚡", nome: "Reserva", blur: true }, { emoji: "📠", nome: "Meia Clássico", blur: true },
];
...
const QuizJogador = () => {
  ...
  useEffect(() => {
    window.scrollTo(0, 0);
    if (startDirect) {
      trackEvent("quiz_start", { quiz_id: "quiz_jogador", source: "lp" });
    }
  }, [startDirect]);

  const handleStart = () => {
    trackEvent("quiz_start", { quiz_id: "quiz_jogador", source: "app" });
    setStep("pergunta");
    setPerguntaIdx(0);
    setRespostas([]);
  ...
  const handleResposta = (opcaoIdx: number) => {
    ...
    if (novasRespostas.length >= PERGUNTAS_JOGADOR.length) {
      setStep("calculando");
      const resultado = calcularJogador(novasRespostas);
      setJogador(resultado);
      trackEvent("quiz_complete", { quiz_id: "quiz_jogador", resultado: resultado.id });
  ...
  const handleAdGate = async () => {
    if (needsAd) {
      trackEvent("quiz_ad_shown", { quiz_id: "quiz_jogador" });
  ...
  const generateImage = useCallback(async (): Promise<File | null> => {
    ...
    try {
      ...
      if (blob) return new File([blob], `quiz-jogador-${jogador?.id || "resultado"}.png`, { type: "image/png" });
    } catch { /* silencioso */ }
    return null;
  }, [jogador]);
  ...
  const generateAndShare = useCallback(async (canal: string) => {
    ...
    trackEvent("quiz_share", { quiz_id: "quiz_jogador", resultado: jogador.id, canal });
    ...
    if (canal === "whatsapp") {
      if (Capacitor.isNativePlatform() && imageFile) {
        try {
          ...
          const saved = await Filesystem.writeFile({ path: `quiz-jogador-${jogador.id}.png`, data: base64, directory: 1 });
      ...
      try {
        if (Capacitor.isNativePlatform()) {
          const { Share } = await import("@capacitor/share");
          await Share.share({ title: jogador.titulo, text: texto, url: storeLink, dialogTitle: "Compartilhar resultado" });
        } else if (navigator.share) {
          const shareData: ShareData = { title: jogador.titulo, text: texto, url: storeLink };
  ...
  if (step === "intro") {
    return (
      <div className="animate-fade-in -mx-4 -mt-6" style={{ background: "#14532d", color: "#fff", fontFamily: "'Inter', sans-serif" }}>
        <SEOHead title="Quiz — Qual jogador de futebol você seria na vida?" description="Responda 7 perguntas e descubra qual posição do futebol combina com seu jeito. São 20 resultados possíveis!" path="/quiz/jogador" />
  ...
  // ═══ TELA: PERGUNTA ═══
  if (step === "pergunta") {
    const pergunta = PERGUNTAS_JOGADOR[perguntaIdx];
    const progresso = ((perguntaIdx) / PERGUNTAS_JOGADOR.length) * 100;
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 space-y-1">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold" style={{ color: "rgba(255,255,255,.5)" }}>Pergunta {perguntaIdx + 1} de {PERGUNTAS_JOGADOR.length}</span>
  ...
  if (step === "calculando") {
    return (
      <div className="animate-fade-in -mx-4 -mt-6 flex flex-col items-center justify-center text-center" style={{ background: "#071410", color: "#fff", minHeight: "100vh" }}>
        <SEOHead title="Calculando sua posição..." noindex />
        ...
        <h2 className="text-xl font-bold mb-2">Calculando sua posição...</h2>
        <p className="text-sm" style={{ color: "rgba(255,255,255,.5)" }}>Analisando suas respostas entre 20 posições</p>
        <div className="mt-6 flex gap-2 animate-pulse">
          {JOGADORES.slice(0, 5).map((j, i) => (
            <span key={i} className="text-2xl" style={{ animationDelay: `${i * 200}ms` }}>{j.emoji}</span>
          ))}
        </div>
      </div>
    );
  ...
  if (step === "ad-gate") {
    return (
      <div className="animate-fade-in -mx-4 -mt-6 flex flex-col items-center justify-center text-center space-y-6" style={{ background: "#071410", color: "#fff", minHeight: "100vh", padding: "48px 20px" }}>
        <SEOHead title="Sua posição está pronta!" noindex />
        ...
        <div>
          <h2 className="text-xl font-bold mb-1">Sua posição está pronta!</h2>
  ...
  if (step === "resultado" && jogador) {
    ...
    return (
      <div className="animate-fade-in -mx-4 -mt-6" style={{ background: "#071410", color: "#fff", minHeight: "100vh" }}>
        <SEOHead title={`Quiz — ${jogador.posicao}`} noindex />
          ...
          <section className="flex flex-col items-center px-5 py-12 text-center">
            ...
            <div ref={resultRef} className="w-full max-w-[400px] flex flex-col items-center">
              ...
              <div className="inline-flex items-center gap-2 rounded-full px-4 py-1.5 mb-3"
                style={{ background: "rgba(250,204,21,.14)", border: "1px solid rgba(250,204,21,.38)" }}>
                <span className="text-xs font-bold uppercase tracking-wider" style={{ color: "#facc15" }}>{jogador.posicao}</span>
              </div>

              {/* Título */}
              <h1 className="mb-6 px-2" style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: "clamp(1.8rem,7vw,3.5rem)", lineHeight: ".9", color: "#facc15", textShadow: "0 0 80px rgba(255,223,0,.35)" }}>
                {jogador.titulo}
              </h1>
            ...
            <div className="w-full max-w-[380px] mt-4 space-y-2.5">
              <p className="text-center mb-1" style={{ fontFamily: "'Barlow Condensed', 'Bebas Neue', sans-serif", fontWeight: 900, fontSize: "clamp(1.4rem,5vw,1.85rem)" }}>
                E seus amigos,<br />qual posição <span style={{ color: "#facc15" }}>eles seriam?</span>
              </p>
              <p className="text-center text-sm mb-5" style={{ color: "rgba(255,255,255,.42)" }}>
                Manda pro grupo e descubra quem é o centroavante da galera ⚽
              </p>
              ...
              <button onClick={() => navigate("/quiz/lenda")}
                ...
                👑 Próximo quiz: Lendas <ChevronRight className="w-5 h-5" />
              </button>

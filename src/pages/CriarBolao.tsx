...
const CriarBolao = () => {
  ...
  // Mata a Mata - pontuação inicial
  const [pontosIniciais, setPontosIniciais] = useState(20);
  ...
  const loadCampeonatos = async () => {
    try {
      const { data, error } = await supabase.from("campeonatos").select("id, nome, nome_popular, logo_url, ativo, tipo").eq("ativo", true).eq("visivel", true).order("nome");

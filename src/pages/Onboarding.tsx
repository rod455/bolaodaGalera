        ...
        {loadingCamps ? (
          ...
          <div className="space-y-1.5">
            {campeonatos.map((c) => {
              ...
              const isNovo = (c.nome_popular || "").toLowerCase().includes("finais estaduais");
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 p-3 rounded-xl border-2 transition-all ${
                    selectedChamps.has(c.id!) ? "border-copa-green-500 bg-copa-green-50" : isNovo ? "border-orange-300 bg-orange-50" : "border-gray-200 bg-white"
                  }`}
                >
                  ...
                  <div className="flex-1 text-left">
                    <div className="flex items-center gap-1.5">
                      <span className="font-semibold text-xs">{c.nome_popular}</span>
                      {isNovo && (
                        <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-orange-100 text-orange-600 animate-pulse">
                          🔥 NOVO
                        </span>
                      )}
                    </div>
                    {isNovo && (
                      <span className="text-[10px] text-orange-500 font-medium">Paulistão, Carioca, Gauchão e Mineiro</span>
                    )}
                  </div>
